...
class CORE_EXPORT QgsVectorDataProvider : public QgsDataProvider, public QgsFeatureSink, public QgsFeatureSource
{
  ...
  public:
    ...
    enum Capability
    {
      ...
      CanDisableSelectAtId = 1 << 27, //!< Provider can disable select at id. Since QGIS 3.16
      CanUseEstimatedMetadata = 1 << 28, //!< Provider can use estimated metadata. Since QGIS 3.16
      CanDisableCheckPrimaryKeyUnicity = 1 << 29, //!< Provider can disable check primary key unicity. Since QGIS 3.16
      CanUseMultiFieldPrimaryKey = 1 << 30, //!< Provider can use multi-field for primary key. Since QGIS 3.16
    };
